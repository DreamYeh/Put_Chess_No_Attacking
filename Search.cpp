...
int Search::Action(unsigned int p) {
	...
	if (p <= WHITE_H_PAWN) {//採用if，以包括更多同樣的兵種	
		...
		if (WPawn_e == b2 && WPawn_g == d2) {
			if (LockB) {
				if (getSq != c1) //居然沒設定到f1，必有問題
					return 0;
			}
			getSq = c1;
			LockB = true;
		...
		if (Init_Position[p] != NO_SQUARE) {
			...
			if (!(attack_bit & Piece_B)) {
				...
				if (!(Piece_Attack_W & Piece_B))
					return Action(BLACK_LEFT_KNIGHT);//跳到黑騎士
			}		
			else return 0;
		}
		//白的兩個主教因為不同色格因此不互相干擾
...
int Search::Statistics(unsigned int p) {
	int Value;
	Square getSq;
	Bitboard possible, attack_bit;
	clock_t start_t = 0, end_t = 0;
	...
	if (p <= WHITE_H_PAWN) {//採用if，以包括更多同樣的兵種	
		...
		while (possible) {
			...
			//若鄰近攻擊格有敵方子 attack & bit_units[]，攻擊到別人，移動A到下一個可能位置
			if (!(attack_bit & Piece_B)) {
				Set_Piece(getSq, WHITE_KING, WHITE);//落子
				Attacked_by_Piece[p] = attack_bit;
				Value = Statistics(p + 1);//繼續找下一個
				Remove_Piece(getSq, WHITE);
			}
		}
		return 0;
		...
		if (Init_Position[p] != NO_SQUARE) {
			...
			if (!(attack_bit & Piece_B)) {
				Put_White_Piece_Done();
				if (!(Piece_Attack_W & Piece_B))
					return Statistics(BLACK_LEFT_KNIGHT);//跳到黑騎士
				return 0;
			}
			else return 0;
		}
		...
		while (possible) {
			...
			if (!(attack_bit & Piece_B)) {
				...
				{//至此放完
					Put_White_Piece_Done();
					if (!(Piece_Attack_W & Piece_B))
						Value = Statistics(BLACK_LEFT_KNIGHT);//跳到黑騎士
				}
				Remove_Piece(getSq, WHITE);
				Attacked_by_Piece[p] = 0;
			}
		...
		//if ((num_Game & 0xffffff) == 0) {//16777216印一次
			printf("%llu=============================\n", num_Game);
			Print_Stat();
		//}
