...
//Dream design
Bitboard ROOK_NEAR_ATTACK[64];
Bitboard BISHOP_NEAR_ATTACK[64];
Bitboard K_NEAR_ATTACK[64];//only test

const Bitboard ROOK_NEARS_ATTACK[64] = {
	0x102, 0x205, 0x40a, 0x814, 0x1028, 0x2050, 0x40a0, 0x8040,
	0x10201, 0x20502, 0x40a04, 0x81408, 0x102810, 0x205020, 0x40a040, 0x804080,
	0x1020100, 0x2050200, 0x40a0400, 0x8140800, 0x10281000, 0x20502000, 0x40a04000, 0x80408000,
	0x102010000, 0x205020000, 0x40a040000, 0x814080000, 0x1028100000, 0x2050200000, 0x40a0400000, 0x8040800000,
	0x10201000000, 0x20502000000, 0x40a04000000, 0x81408000000, 0x102810000000, 0x205020000000, 0x40a040000000, 0x804080000000,
	0x1020100000000, 0x2050200000000, 0x40a0400000000, 0x8140800000000, 0x10281000000000, 0x20502000000000, 0x40a04000000000, 0x80408000000000,
	0x102010000000000, 0x205020000000000, 0x40a040000000000, 0x814080000000000, 0x1028100000000000, 0x2050200000000000, 0x40a0400000000000, 0x8040800000000000,
	0x201000000000000, 0x502000000000000, 0xa04000000000000, 0x1408000000000000, 0x2810000000000000, 0x5020000000000000, 0xa040000000000000, 0x4080000000000000,
};

const Bitboard BISHOP_NEARS_ATTACK[64] = {
	0x200, 0x500, 0xa00, 0x1400, 0x2800, 0x5000, 0xa000, 0x4000,
	0x20002, 0x50005, 0xa000a, 0x140014, 0x280028, 0x500050, 0xa000a0, 0x400040,
	0x2000200, 0x5000500, 0xa000a00, 0x14001400, 0x28002800, 0x50005000, 0xa000a000, 0x40004000,
	0x200020000, 0x500050000, 0xa000a0000, 0x1400140000, 0x2800280000, 0x5000500000, 0xa000a00000, 0x4000400000,
	0x20002000000, 0x50005000000, 0xa000a000000, 0x140014000000, 0x280028000000, 0x500050000000, 0xa000a0000000, 0x400040000000,
	0x2000200000000, 0x5000500000000, 0xa000a00000000, 0x14001400000000, 0x28002800000000, 0x50005000000000, 0xa000a000000000, 0x40004000000000,
	0x200020000000000, 0x500050000000000, 0xa000a0000000000, 0x1400140000000000, 0x2800280000000000, 0x5000500000000000, 0xa000a00000000000, 0x4000400000000000,
...
void init_get_near() {
	int n7, n8, n9,
		n4, n6,
		n1, n2, n3;//keyboard 9 num

	for (int pos = 0; pos < 64; pos++) {
		ROOK_NEAR_ATTACK[pos] = 0;
		BISHOP_NEAR_ATTACK[pos] = 0;
		K_NEAR_ATTACK[pos] = 0;

		int x = pos % 8;
		int y = pos / 8;

		n7 = pos - 1 + 8;	n8 = pos + 8;	n9 = pos + 1 + 8;
		n4 = pos - 1;	n6 = pos + 1;
		n1 = pos - 1 - 8;	n2 = pos - 8;	n3 = pos + 1 - 8;
		if (x == 0) {//n147不合法
			n1 = -1; n4 = -1; n7 = -1;
		}
		if (x == 7) {//n369不合法
		...
		if (n1 >= 0 && n1 < 64)
			BISHOP_NEAR_ATTACK[pos] |= SQUARE_BB[n1];
		if (n2 >= 0 && n2 < 64)
			ROOK_NEAR_ATTACK[pos] |= SQUARE_BB[n2];
		if (n3 >= 0 && n3 < 64)
			BISHOP_NEAR_ATTACK[pos] |= SQUARE_BB[n3];
		if (n4 >= 0 && n4 < 64)
			ROOK_NEAR_ATTACK[pos] |= SQUARE_BB[n4];
		if (n6 >= 0 && n6 < 64)
			ROOK_NEAR_ATTACK[pos] |= SQUARE_BB[n6];
		if (n7 >= 0 && n7 < 64)
			BISHOP_NEAR_ATTACK[pos] |= SQUARE_BB[n7];
		if (n8 >= 0 && n8 < 64)
			ROOK_NEAR_ATTACK[pos] |= SQUARE_BB[n8];
		if (n9 >= 0 && n9 < 64)
			BISHOP_NEAR_ATTACK[pos] |= SQUARE_BB[n9];
		K_NEAR_ATTACK[pos] = ROOK_NEAR_ATTACK[pos] | BISHOP_NEAR_ATTACK[pos];
		printf("0x%llx,\n", ROOK_NEAR_ATTACK[pos]);
	}
}
